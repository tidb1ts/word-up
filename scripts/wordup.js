...
function render() {
    ...
    $("#time-remaining").text(model.secondsRemaining - 1);
...
function wordSubmissionChip(wordSubmission) {
    ...
    if (wordSubmission.hasOwnProperty("isRealWord")) {
        ...
        if (wordSubmission.isRealWord) {
            scoreChip.text(" " + wordScore(wordSubmission))
...
function wordScore(word) {
    var wordTotal = 0,
        wordString = "";
    if (typeof word == "object") {wordString = word.word}
    ...
    letterScores.forEach(function(score){
        wordTotal += score;
    });
    // add the words' score to the model object
    word.pointValue = wordTotal;
...
function currentScore() {
    ...
    var wordScores = model.wordSubmissions.map(function(submission) {
        return submission.pointValue;
    });
    // TODO 20
    // return the total sum of the word scores
    wordScores.forEach(function(score){
